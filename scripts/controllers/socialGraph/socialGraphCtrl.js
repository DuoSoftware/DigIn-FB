...
routerApp.controller('socialGraphCtrl', function ($scope, config, fbGraphServices,ngToast,
    ...
    $scope.changeTimeRange = function () {
        ...
        //alert(typeof(since));
        if (since > until) {
            notifications.toast(0, "From Date should be less than the To Date");
            return;
        }
    ...
    $scope.onClickViewTable = function (_type) {
        ...
        if (_type == 'sentiment') {
            $mdDialog.show({
                controller: viewSentimentCtrl,
                templateUrl: 'views/socialGraph/viewSentimentData.html',
                clickOutsideToClose: true,
                locals: {
                    jsonData: $scope.sentimentConfigData,
                    pageName: $scope.page
                }
            });
            return;            
...
function viewSentimentCtrl($scope, $mdDialog, jsonData, pageName) {
    $scope.tableData = [];
    $scope.pageName = pageName;
    angular.forEach(jsonData,function(data) {
        data[0] = moment(data[0]).format('LL');
...
}).directive('diginWordCloud', function() {
    return {
        restrict: 'E',
        transclude: true,
        scope: {
